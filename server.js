...
app.get('/api/chats', auth, (req, res) => {
  ...
  const result = chats.map(chat => {
    ...
    const members = db.prepare(`SELECT u.id,u.username,u.first_name,u.last_name,u.avatar_color,cm.role FROM users u JOIN chat_members cm ON cm.user_id=u.id WHERE cm.chat_id=?`).all(chat.id);
...
app.delete('/api/chats/:id/members/:userId', auth, (req, res) => {
  ...
  if (userId !== req.user.id) {
    const myRole = db.prepare('SELECT role FROM chat_members WHERE chat_id=? AND user_id=?').get(id, req.user.id);
